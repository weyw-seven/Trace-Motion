...
/*
 * Environment compensation for the measured feed-forward intercept B.
 *
 * PWM_MIN is the only field normally changed for a new floor/load.  A single
 * scale factor is derived from the smallest calibrated B, so every wheel and
 * direction keeps its measured relative characteristic:
 *
 *     B_effective[i] = clamp(B_calibrated[i] * scale, 0, B_MAX)
 *     scale = max(1, PWM_MIN / min(B_calibrated))
 *
 * This replaces the old adaptive startup boost.  Set PWM_MIN to zero to use
 * the unscaled calibration.  MOTOR_ENCODER_FILTER_ALPHA remains exclusively
 * a measurement filter and is intentionally unrelated to this calculation.
 */
#ifndef MOTOR_PWM_MIN
#ifdef PWM_MIN
#define MOTOR_PWM_MIN                  PWM_MIN
#else
#define MOTOR_PWM_MIN                  100.0f
#endif
#endif
...
#define MOTOR_A_PID_KI                 0.0f
#endif
#ifndef MOTOR_A_PID_KD
#define MOTOR_A_PID_KD                 0.00005f
#endif
...
#define MOTOR_B_PID_KI                 0.0f
#endif
#ifndef MOTOR_B_PID_KD
#define MOTOR_B_PID_KD                 0.00005f
#endif
...
#define MOTOR_D_PID_KI                 0.0f
#endif
#ifndef MOTOR_D_PID_KD
#define MOTOR_D_PID_KD                 0.00005f
#endif
...
/* Diagnostic stall detection. These are deliberately conservative first-run
 * thresholds; the automatic per-device calibration will replace them later. */
...
#define MOTOR_STALL_DETECT_TIME_MS          300U
#endif
    ...
    float target_speed)
{
    ...
    */
    ...
    /*
     * Keep a moving wheel above its measured running deadband.  This is
     * separate from startup_boost_pwm: a wheel that is already moving needs
     * less PWM than a wheel breaking away from rest.
     */
    output =
        motor_clampf(
            unsat,
